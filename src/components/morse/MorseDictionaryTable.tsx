...
export function MorseDictionaryTable({ filter = "all" }: MorseDictionaryTableProps) {
  const entries =
    filter === "all"
      ? MORSE_DICTIONARY
      : MORSE_DICTIONARY.filter((e) => e.category === filter);

  const letters = entries.filter((e) => e.category === "letter");
  const numbers = entries.filter((e) => e.category === "number");
  const symbols = entries.filter((e) => e.category === "symbol");

  const groups =
    filter !== "all"
      ? [{ label: filter, items: entries }]
      : [
          { label: "Letters (A–Z)", items: letters },
          { label: "Numbers (0–9)", items: numbers },
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-700/60 bg-slate-800/80">
                      <th className="px-4 py-3 text-left font-semibold text-slate-400">Character</th>
                      <th className="px-4 py-3 text-left font-semibold text-slate-400">Morse Code</th>
                      <th className="px-4 py-3 text-left font-semibold text-slate-400">Pattern</th>
